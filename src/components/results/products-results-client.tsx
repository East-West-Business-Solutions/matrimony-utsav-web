...
/** Card is non-link until product detail routes exist; CTA anchored at bottom. */
function ProductCard({ product }: { product: Product }) {
  const { name, brand, city, category, price, rating } = product;

  return (
    <article
      className="group flex h-full flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md"
      aria-label={name}
    >
      {/* Image or category fallback */}
      <div className="relative flex h-40 w-full shrink-0 items-center justify-center bg-slate-100">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className="mt-1 text-sm text-slate-500">by {brand}</p>
...
export default function ProductsResultsClient() {
