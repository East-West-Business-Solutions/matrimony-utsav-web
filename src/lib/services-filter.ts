export type ServiceSort = "recommended" | "rating" | "price_asc" | "price_desc";

export type ServiceFilterShape = {
  city: string;
  category: string;
  price: number;
  rating: number;
...
export function parseServiceQueryParams(searchParams: URLSearchParams) {
  ...
  const sortRaw = (searchParams.get("sort") ?? "recommended").trim() as ServiceSort;
  const sort: ServiceSort =
...
export function filterAndSortServices<T extends ServiceFilterShape>(
  items: T[],
